  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        if (!data.session) {
          setMsg("Compte crÃƒÆ’Ã‚Â©ÃƒÆ’Ã‚Â©. VÃƒÆ’Ã‚Â©rifie ton email si la confirmation est activÃƒÆ’Ã‚Â©e, puis reconnecte-toi.");
        } else {
          router.push(next);
          router.refresh();
        }
      ...
      <div className="mx-auto max-w-md space-y-6">
        ...
        <form className="rounded-md border p-4 space-y-4" onSubmit={onSubmit}>
          ...
          <button className="w-full rounded-md bg-black px-4 py-2 text-white disabled:opacity-50" disabled={loading}>
            {loading ? "..." : mode === "login" ? "Se connecter" : "CrÃƒÆ’Ã‚Â©er le compte"}
          </button>
        </form>

        <a className="text-sm underline" href="/">
          ÃƒÂ¢Ã¢â‚¬Â Ã‚Â Retour
        </a>
      </div>
    </main>
  );
